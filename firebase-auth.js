...
(function(){
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    let resolved=false;
    dhAuth.onAuthStateChanged(function(user){resolved=true;document.body.classList.remove('auth-loading');if(user){login.classList.add('hidden');app.classList.remove('hidden');showPage('dashboard');const su=document.querySelector('#signedInUser');if(su)su.textContent='Signed in: '+(user.email||'')}else{app.classList.add('hidden');login.classList.remove('hidden');const p=document.querySelector('#password');if(p)p.value=''}});
    setTimeout(function(){if(!resolved){document.body.classList.remove('auth-loading');app.classList.add('hidden');login.classList.remove('hidden')}},5000);
  });
})();