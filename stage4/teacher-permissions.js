...
(function(){
  ...
  const readUser=()=>{try{return JSON.parse(localStorage.getItem(KEY)||'null')||defaultUser}catch(e){return defaultUser}};
  const saveUser=u=>localStorage.setItem(KEY,JSON.stringify(u));
  const can=p=>!!permissions[readUser().role]?.[p];
  const labels={dashboard:'Dashboard',transactions:'Income / Expenses',daily:"Today's Total",students:'Students Data',fees:'Student Fees',accounts:'Accounts Overview',teachers:'Teacher Management',permissions:'Role & Permissions'};
  ...
  window.navigate=function(page){
    if(!can(page)){
      openModal(`<div class="modal-head"><h3>Access Restricted</h3><button class="close" onclick="closeModal()">×</button></div><div class="empty"><strong>Admin access required</strong>This section is not available for the current role.</div><div class="modal-actions"><button class="btn btn-light" onclick="closeModal()">Close</button></div>`);
      return;
    }
  ...
  const renderWithPermissions=window.render;
  window.render=function(){
    if(state.page==='permissions') return renderPermissions();
